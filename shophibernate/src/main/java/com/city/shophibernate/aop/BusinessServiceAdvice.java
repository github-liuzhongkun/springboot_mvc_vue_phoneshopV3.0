package com.city.shophibernate.aop;
...
public class BusinessServiceAdvice {
	...
	@Before(value="execution(* com.city.shophibernate.service.impl.*.*(..))")
	...
	@AfterReturning(value="execution(* com.city.shophibernate.service.impl.*.*(..))",returning="result")
	...
	@After(value="execution(* com.city.shophibernate.service.impl.*.*(..))")
	public void methodAfter(JoinPoint jp) throws Exception{
		System.out.println("业务层方法后（after）切面代码执行。。。。。");
		System.out.println("切入类："+jp.getTarget().getClass().getName()+" 方法:"+jp.getSignature().getName());
	}
	
	@Around(value="execution(* com.city.shophibernate.service.impl.*.*(..))")
	...
	@AfterThrowing(value="execution(* com.city.shophibernate.service.impl.*.*(..))",throwing="ex")
